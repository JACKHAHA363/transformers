...
from .configuration_robin import RobinConfig
...
ROBIN_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "EleutherAI/gpt-neox-20b",
    # See all GPTNeoX models at https://huggingface.co/models?filter=gpt_neox
]


class RobinPretrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = RobinConfig
    base_model_prefix = "robin"
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, RobinModel):
            module.gradient_checkpointing = value


class RobinAttention(nn.Module):
...
class RobinLayer(nn.Module):
    """ Each layer with adaptor """
    def __init__(self, config):
        super().__init__()
        ...
        self.attention = AdapterWrapper(
            attn_block=RobinAttention(config),
            dim=config.hidden_size,
            downsample_factor=config.adapter_downsample_factor)
        self.mlp = AdapterWrapper(
            attn_block=RobinMLP(config),
            dim=config.hidden_size,
            downsample_factor=config.adapter_downsample_factor)
...
class RobinModel(RobinPretrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([RobinLayer(config) for _ in range(config.num_hidden_layers)])
...
class RobinForCausalLM(RobinPretrainedModel):
    _tied_weights_keys = ["embed_out.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.gpt_neox = RobinModel(config)
